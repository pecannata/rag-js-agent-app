...
async function initializeBlogTable(): Promise<boolean> {
  ...
  try {
    ...
    if (checkResult.success) {
      ...
      // Check if we need to update the identity column to start from 500
      try {
        const maxIdQuery = `SELECT MAX(id) as max_id FROM blog_posts`;
        const maxIdResult = await executeOracleQuery(maxIdQuery);
        
        if (maxIdResult.success && maxIdResult.data && maxIdResult.data.length > 0) {
          const maxId = maxIdResult.data[0].max_id || 0;
          console.log('📊 Current max ID in blog_posts:', maxId);
          
          // If max ID is below 500, we need to restart the sequence from 500
          if (maxId < 500) {
            console.log('🔄 Updating identity column to start from 500...');
            
            // In Oracle, we need to modify the identity column
            const updateIdentityQuery = `ALTER TABLE blog_posts MODIFY id GENERATED BY DEFAULT AS IDENTITY (START WITH 500 INCREMENT BY 1)`;
            const updateResult = await executeOracleQuery(updateIdentityQuery);
            
            if (updateResult.success) {
              console.log('✅ Identity column updated to start from 500');
            } else {
              console.log('⚠️ Could not update identity column (may require recreating):', updateResult.error);
              // Try alternative approach: restart the sequence manually
              try {
                // First, get the identity sequence name
                const sequenceQuery = `
                  SELECT sequence_name 
                  FROM user_tab_identity_cols 
                  WHERE table_name = 'BLOG_POSTS' AND column_name = 'ID'
                `;
                const sequenceResult = await executeOracleQuery(sequenceQuery);
                
                if (sequenceResult.success && sequenceResult.data && sequenceResult.data.length > 0) {
                  const sequenceName = sequenceResult.data[0].sequence_name;
                  console.log('🔍 Found identity sequence:', sequenceName);
                  
                  // Drop and recreate the sequence to start from 500
                  const dropSequenceQuery = `DROP SEQUENCE ${sequenceName}`;
                  const createSequenceQuery = `CREATE SEQUENCE ${sequenceName} START WITH 500 INCREMENT BY 1`;
                  
                  await executeOracleQuery(dropSequenceQuery);
                  const createSeqResult = await executeOracleQuery(createSequenceQuery);
                  
                  if (createSeqResult.success) {
                    console.log('✅ Sequence recreated to start from 500');
                  } else {
                    console.log('⚠️ Could not recreate sequence:', createSeqResult.error);
                  }
                }
              } catch (seqError) {
                console.log('⚠️ Alternative sequence update failed:', seqError);
              }
            }
          }
        }
      } catch (idCheckError) {
        console.log('⚠️ Could not check max ID:', idCheckError);
      }
  ...
  const createTableQuery = `
    CREATE TABLE blog_posts (
      id NUMBER GENERATED BY DEFAULT AS IDENTITY (START WITH 500 INCREMENT BY 1),
...
export async function GET(request: NextRequest) {
  try {
    // Initialize table and update identity column if needed
