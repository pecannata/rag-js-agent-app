...
export default function OracleDatabaseTool({ cohereApiKey, onSqlResult, onUserContext }: OracleDatabaseToolProps) {
  ...
  const executeReActDecision = async () => {
    ...
    try {
      ...
      console.log("🤖 Starting ReAct decision process...");
      setProgress(20);
...
Start your response with either "YES" or "NO" followed by your reasoning.`;
      ...
      console.log("🤖 ReAct raw response:", reactData.response);
      
      // Parse the decision
      const isApproved = responseText.startsWith('yes');
      
      console.log("🤖 ReAct decision:", isApproved ? "APPROVED" : "DECLINED");
      ...
      if (isApproved) {
        console.log("✅ ReAct approved - executing SQL query");
        await executeDatabaseQuery();
      } else {
        console.log("❌ ReAct declined - NOT executing SQL query");
        setProgress(100);
  ...
  const executeDatabaseQuery = async () => {
    try {
      ...
      if (dbResponse.ok) {
        ...
        if (dbResponse.status === 403 && dbData.reactDecision) {
          // ReAct decision rejected the query at the API level
